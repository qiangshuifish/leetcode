...
public class SmsCacheAnnotationParser implements CacheAnnotationParser, Serializable {

    private static final Logger logger = LoggerFactory.getLogger(SmsCacheAnnotationParser.class);
...
//        return parseCacheAnnotations(defaultConfig, type);
    ...
    private Collection<CacheOperation> parseCacheAnnotations(DefaultCacheConfig cachingConfig, AnnotatedElement ae) {
        ...
        if(ae instanceof  Class){
            logger.error("暂时不支持类级别的缓存" + ((Class) ae).getName());
        }

        Collection<CacheOperation> ops = null;

        Collection<SmsSpringRedisCache> smsCacheables = AnnotatedElementUtils.getAllMergedAnnotations(ae, SmsSpringRedisCache.class);
        if (!smsCacheables.isEmpty()) {
            ops = lazyInit(ops);
            for (SmsSpringRedisCache smsSpringRedisCache : smsCacheables) {
                ops.add(parseSmsCacheAnnotation(ae, cachingConfig, smsSpringRedisCache));
            }
        }
        ...
        if (!cacheables.isEmpty()) {
            ...
            for (Cacheable cacheable : cacheables) {
                ops.add(parseSmsCacheAnnotation(ae, cachingConfig, cacheable));
            }
        }
    ...
    Collection<CacheOperation> parseCachingAnnotation(AnnotatedElement ae, DefaultCacheConfig defaultConfig, Caching caching) {
        ...
        if (!ObjectUtils.isEmpty(cacheables)) {
            ops = lazyInit(ops);
            for (Cacheable cacheable : cacheables) {
                ops.add(parseSmsCacheAnnotation(ae, defaultConfig, cacheable));
            }
        }
    ...
    /**
     * 解析 Cacheable 和  SmsSpringRedisCache
     */
    private CacheableOperation parseSmsCacheAnnotation(AnnotatedElement ae, DefaultCacheConfig defaultConfig, Object cacheAnnotation){
        CacheableOperation operation = null;
        if(ae instanceof Method){
            operation = parseSmsCacheAnnotation(ae,defaultConfig);
        }

        if(Objects.nonNull(operation)){
            return operation;
        }
        return parseCacheableAnnotation(ae,defaultConfig,(Cacheable)cacheAnnotation);
    }

    private CacheableOperation parseSmsCacheAnnotation(
            AnnotatedElement ae, DefaultCacheConfig defaultConfig) {

        if(!(ae instanceof Method)){
            return null;
        }

        String cacheName = getCacheName((Method) ae);

        SmsCacheConfigVO configVo = SmsSpringRedisCacheRegister.getCacheConfigVo(cacheName);
        if(Objects.isNull(configVo)){
            return null;
        }

        CacheableOperation.Builder builder = new CacheableOperation.Builder();
        builder.setUnless(configVo.getUnless());
        builder.setSync(configVo.getSync());
        builder.setName(ae.toString());
        builder.setCacheNames(configVo.getCacheNames());
        builder.setKey(configVo.getKey());
        builder.setKeyGenerator(configVo.getKeyGenerator());
        builder.setCacheManager(configVo.getCacheManager());
        builder.setCacheResolver(configVo.getCacheResolver());
        builder.setCondition(configVo.getCondition());
